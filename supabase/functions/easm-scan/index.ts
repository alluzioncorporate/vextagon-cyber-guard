      ...
      try {
        ...
        if (data.Answer) {
          for (const answer of data.Answer) {
            // Clean up trailing dots and avoid duplicates
...
// SSL check via Deno TLS connection (Free)
async function checkSSL(domain: string) {
  try {
    ...
    try {
      // Try to get certificate details (available in Deno 2.0+)
    ...
    if (certInfo) {
      ...
      return {
        ...
        protocol: 'TLS 1.3',
        grade,
        expiresAt: validTo ? validTo.toLocaleDateString('pt-BR') : 'N/A',
      };
    }
    
    // Connection OK but no cert details available
    return {
      valid: true,
...
// Security headers via HTTP fetch (Free)
async function checkHeaders(domain: string): Promise<Record<string, { present: boolean; value: string | null }>> {
  ...
  try {
    ...
    await res.body?.cancel(); // Consume/discard body
    
    for (const [header] of Object.entries(headers)) {
      const value = res.headers.get(header);
      if (value) {
        headers[header] = { present: true, value };
      }
    }
  } catch {
    // Try HTTP fallback
...
// TCP port scan via Deno.connect (Free)
...
// Shodan lookup (Optional - requires API key)
async function lookupShodan(domain: string, apiKey: string) {
  try {
    ...
    return {
      ...
      services: ((data.data as any[]) || []).slice(0, 6).map((s: any) => ({
        port: s.port,
...
// Generate vulnerabilities from scan findings
function detectVulnerabilities(
  ...
  for (const p of openPorts) {
    if (dangerous[p.port]) {
      vulns.push({ id: id('PORT'), ...dangerous[p.port] });
    }
  }
  ...
  if (!ssl.valid) {
    vulns.push({ 
      id: id('SSL'), 
      severity: 'critical', 
      title: 'SSL Inválido ou Expirado', 
      description: 'Certificado expirado expõe usuários a ataques man-in-the-middle.',
      affected: 'SSL/TLS'
    });
  }
  ...
  if (missingHSTS && missingCSP) {
    vulns.push({ 
      id: id('HDR'), 
      severity: 'high', 
      title: 'HSTS e CSP Ausentes', 
      description: 'Sem HSTS e CSP o site fica vulnerável a ataques de downgrade e XSS.',
    ...
    vulns.push({ 
      ...
      description: 'Sem HSTS o navegador pode acessar o site via HTTP inseguro.',
      affected: 'Security Headers'
    });
  } else if (missingCSP) {
    vulns.push({ 
      ...
      severity: 'medium', 
      title: 'CSP Ausente', 
      description: 'Content Security Policy reduz riscos de XSS e injeção de conteúdo.',
      affected: 'Security Headers'
    });
  }
  
  for (const cve of shodanVulns.slice(0, 5)) {
    vulns.push({ 
      id: cve, 
      severity: 'critical', 
      title: `CVE Detectado: ${cve}`, 
      description: 'Vulnerabilidade conhecida detectada via Shodan Intelligence.',
      affected: 'System'
    });
  }
...
function calculateScore(ssl: any, headers: Record<string, any>, ports: any[], vulns: any[]) {
  ...
  score -= missing * 4; // max -28 for 7 headers
  
  const criticalOpen = ports.filter(p => p.status === 'open' && p.risk === 'critical').length;
  const highOpen = ports.filter(p => p.status === 'open' && p.risk === 'high').length;
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Run all free scans in parallel
    ...
    if (shodanKey) {
      ...
      if (shodanData) {
        ...
        shodan = {
          ip: shodanData.ip,
          os: shodanData.os,
          organization: shodanData.organization,
          isp: shodanData.isp,
    ...
    // If Shodan not available, use DNS A record for IP
    if (!shodan) {
      const aRecord = dns.records.find(r => r.type === 'A');
      shodan = {
        ip: aRecord?.value || 'N/A',
        os: 'N/A',
        organization: 'N/A (Shodan não ativado)',
        isp: 'N/A',
        lastUpdate: 'N/A',
        services: [],
        vulns: []
